...
void BitcoinMiner(CWallet* pwallet, CBlockProvider* block_provider, unsigned int thread_id)
{
    ...
    CReserveKey reservekey(pwallet); // Ignored while pool mining
	...
	if (block_provider == NULL) {
		...
		MilliSleep(20000);
		continue;
	} else if (old_hash == pblock->GetHash()) {
	...
	uint256 testHash;
		...
		for(int i=0;i<1;i++){
			...
			//printf("hash2 %s\n", pblock->GetHash().ToString().c_str());
                    ...
                    if(testHash != pblock->GetHash()) printf("test hash not equal to block hash!");

                    SetThreadPriority(THREAD_PRIORITY_NORMAL);
                    if(block_provider == NULL)
						CheckWork(pblock, *pwalletMain, reservekey);
					else block_provider->submitBlock(pblock);
                    SetThreadPriority(THREAD_PRIORITY_LOWEST);
                    break;
                }
            }
            ...
            if (pindexPrev != pindexBest/* || (block_provider != NULL && GetTime() - nStart > 200)*/)
                break;
			if (thread_id == 0 && block_provider != NULL && (GetTime() - nStart) > 60 * 18) { // 18 minutes no update? something's wrong -> reconnect!
            ...
            if (fTestNet)
            {
                ...
                hashTarget = CBigNum().SetCompact(0x200fffff).getuint256();
            }
        }
    } }
    catch (boost::thread_interrupted)
